...
public class Reader {
    ...
    private static final Pattern floatRE =
    ...
    private MalKeyword read_keyword() {
        String k = next();
        return new MalKeyword(k.substring(1));
    }

    private MalString read_string() {
        String s = next();
        s = s.substring(1, s.length() - 1);
        return new MalString(s.replace("\\\"", "\"").replace("\\n", "\n"));
    }

    private MalVector read_vector() {
    ...
    private MalObject read_atom() {
        ...
        } else if (floatRE.matcher(code).matches()){
            return new MalFloat(Double.parseDouble(code));
